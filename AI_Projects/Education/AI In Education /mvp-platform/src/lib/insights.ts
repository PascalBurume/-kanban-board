...
export async function buildInsights(teacherId: string, classId?: string) {
  const studentIds = await teacherStudentIds(teacherId, classId);
  if (studentIds.length === 0) {
    return { kpis: { questions: 0, students: 0, themes: 0, peakHour: null }, clusters: [], topQuestions: [], heatmap: { cells: [], max: 0 }, usageByHour: Array(24).fill(0) };
  }

  const rows = await prisma.copilotMessage.findMany({
    where: { role: "user", thread: { studentId: { in: studentIds } } },
  ...
  const msgs: Msg[] = rows.map((r) => ({
    ...
    icon: r.thread.lesson.module.subject.icon,
  }));
  ...
  return {
    kpis: { questions: N, students: new Set(msgs.map((m) => m.studentId)).size, themes: clusters.length, peakHour },
    clusters,
    topQuestions,
