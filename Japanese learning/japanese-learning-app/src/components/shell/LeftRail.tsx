  ...
  return (
    <aside className="hidden md:flex w-[64px] lg:w-[208px] shrink-0 flex-col gap-1 border-r border-ink-3/40 bg-paper-2 p-3">
      ...
      <nav className="flex flex-col gap-0.5">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={`flex size-6 shrink-0 items-center justify-center rounded-sm border text-[13px] jp ${
                  isActive
      ...
      <div className="mt-auto border-t border-dotted border-ink-3/40 pt-3">
        <Link
          href="/me"
          className="flex items-center gap-2 rounded-md p-1 hover:bg-paper-3/60"
        >
          <div className="flex size-7 items-center justify-center rounded-full border border-ink/70 bg-paper-3 text-[13px]">
            {user.name[0]}
          </div>
